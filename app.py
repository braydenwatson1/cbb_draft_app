...
def reset_draft():
    for key in ["stage", "teams", "available_pool", "pick_order", "current_pick",
                ...
                "pick_select_nonce", "draft_search", "bpa_mode", "bpa_reveal_count",
                "setup_bpa_enabled", "setup_cpu_difficulty"]:
...
def cpu_make_pick(team_idx):
    ...
    # Difficulty changes the candidate pool before the normal CPU evaluation.
    # Empty restricted pools fall back to every available player.
    difficulty = st.session_state.get("cpu_difficulty", "Hard")
    restricted_sub = None
    if difficulty == "Medium":
        if random.random() >= 0.5:
            restricted_sub = sub[sub["draft_pick"].isna() | (sub["draft_pick"] > 30)]
        ...
        if roll < 0.15:
            ...
            restricted_sub = sub[sub["draft_pick"].isna()]
    if restricted_sub is not None and not restricted_sub.empty:
        sub = restricted_sub
...
def commit_pick(team_idx, player_id, *, from_user=False):
    ...
    st.session_state.bpa_reveal_count = BPA_TOP_N
    if from_user:
...
def best_available_by_position(rank_col, n=BPA_TOP_N):
    ...
    for pos in ["PG", "SG", "SF", "PF", "C"]:
        ...
        if pos_df.empty or rank_col not in pos_df.columns:
            ranked[pos] = []
            continue
        # Keep BPA team-agnostic and deterministic, while accounting for a
        # portion of the talent, pedigree, and winning signals CPUs value.
        bpa_score = pos_df[rank_col].fillna(0.0).copy()
        if "z_porpag" in pos_df.columns:
            bpa_score = bpa_score + 1.1 * pos_df["z_porpag"].fillna(0.0)
        if "z_draft" in pos_df.columns:
            bpa_score = bpa_score + 0.8 * pos_df["z_draft"].fillna(0.0)
        if "z_team_strength" in pos_df.columns:
            bpa_score = bpa_score + 0.4 * pos_df["z_team_strength"].fillna(0.0)
        top = pos_df.assign(_bpa_score=bpa_score).sort_values(
            "_bpa_score", ascending=False, na_position="last"
        ).head(n)
...
def build_tournament_prompt():
    ...
    lines.append(
        ...
        "generally respect team strength). Don't simply let the user win every game, unless they truly deserve it. Output in Markdown."
    )
    ...
    lines.append(
        ...
        " recap that references each team's hidden strategy now that the "
        ...
        f"5. Finish with a savage roast of **{worst_name}**'s roster and strategy "
...
def render_setup():
    ...
    with st.container(border=True):
        ...
        with options_left:
            bpa_enabled = st.toggle(
                "Show Best Available board during draft",
                value=st.session_state.get("bpa_enabled", True),
                key="setup_bpa_enabled",
            )
        with options_right:
            cpu_difficulty = st.segmented_control(
                "CPU difficulty",
                options=["Easy", "Medium", "Hard"],
                default=st.session_state.get("cpu_difficulty", "Hard"),
                required=True,
                key="setup_cpu_difficulty",
            )
    ...
    if start_clicked:
        ...
        st.session_state.bpa_enabled = bpa_enabled
        st.session_state.bpa_reveal_count = BPA_TOP_N
        st.session_state.cpu_difficulty = cpu_difficulty
...
def render_bpa_board():
    ...
    reveal_count = st.session_state.get("bpa_reveal_count", BPA_TOP_N)
    board = best_available_by_position(rank_col, n=reveal_count)
    cols = st.columns(5)
    for col, pos in zip(cols, ["PG", "SG", "SF", "PF", "C"]):
        with col:
            st.markdown(f"**{pos}**")
    ...
    with controls[0]:
        if st.button("Show more", key="bpa_show_more", width="stretch"):
            st.session_state.bpa_reveal_count = reveal_count + BPA_TOP_N
            st.rerun()
    if reveal_count > BPA_TOP_N:
        with controls[1]:
            if st.button("Hide", key="bpa_hide", width="stretch"):
                st.session_state.bpa_reveal_count = BPA_TOP_N
                st.rerun()
...
def render_draft():
    ...
    st.divider()
    if team_on_clock == user_idx and st.session_state.get("bpa_enabled", True):
        render_bpa_board()
...
if __name__ == "__main__":
    main()
>>>>>>> 77a2a91fd39cb31855f3b1510a8052a57b50d19b
